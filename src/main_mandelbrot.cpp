...
void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, bool useGPU, unsigned int antialiasing);
...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(16, 8, width, height),
                        gpu_results_gpu,
                        width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
        ...
        size_t flopsInLoop = 10 * antialiasing * antialiasing;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        size_t gflops = 1000*1000*1000;
    ...
    bool useGPU = true;
    renderInWindow(centralX, centralY, iterationsLimit, useGPU, antialiasing);
