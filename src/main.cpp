...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &memorySize, nullptr);
            std::cout << "        Device memory size: " << memorySize / 1024 / 1024 << " MB" << std::endl;

            // - Еще пару или более свойств устройства, которые вам покажутся наиболее интересными
            size_t openclVersionSize = 0;
            clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION, 0, nullptr, &openclVersionSize);
            std::vector<unsigned char> openclVersion(openclVersionSize, 0);
            clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION, openclVersionSize, openclVersion.data(), nullptr);
            std::cout << "        Device OpenCL C version: " << openclVersion.data() << std::endl;

            cl_ulong cacheMemorySize = 0;
            clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(cl_ulong), &cacheMemorySize, nullptr);
            std::cout << "        Device cache-memory size: " << cacheMemorySize / 1024 << " KB" << std::endl;
        }
    }
